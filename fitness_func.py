import pickle
import numpy
def calculate_return_risk(func, n_stocks, df_dict, stocknames, iter, month,factor_list,total_stocks):

    df = df_dict[stocknames[0]]
    date_60 = df.index.values[(iter) * 12 + month:((iter) * 12 + 12 + month)]

    start_date_index = (iter) * 12 + month

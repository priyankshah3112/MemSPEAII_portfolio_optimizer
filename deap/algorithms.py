...
def eaMuPlusLambda(start_gen,population, toolbox, mu, lambda_, cxpb, mutpb, ngen,
    ...
    *ngen* generations are done, the algorithm returns a tuple with the final
    ...
    # Evaluate the individuals with an invalid fitness
    invalid_ind = [ind for ind in population if not ind.fitness.valid]
    # print("Len of invalid 1st time: "+ str(len(invalid_ind)))

    fitnesses = toolbox.map(toolbox.evaluate,  ((inv,df_dict) for inv in invalid_ind))
    ...
    for gen in range(start_gen+1, ngen+1):
        ...
        if halloffame is not None:
            halloffame.update(offspring)
        # logging.warning("Start selection")
        # Select the next generation population
        population[:] = toolbox.select(population + offspring, mu,toolbox)

        # logging.warning("End selection")
            ...
            if (len(non_dominated_pop) > 0):
                while(len(memory_pop)<=mem_max):
                    ...
                    if(len(non_dominated_pop)<=0):
                        break
            if(len(non_dominated_pop)>0):
                # logging.warning("Memory full")
                new_memory=[]
                old_memory= toolbox.clone(memory_pop)
                old_non_dominated_pop= toolbox.clone(non_dominated_pop)
                prob_memory=toolbox.map(toolbox.evaluate_prob,((memi,df_dict,iter,month) for memi in memory_pop))
                prob_non_dominated= toolbox.map(toolbox.evaluate_prob, ((nd_ind, df_dict, iter, month) for nd_ind in non_dominated_pop))
                fit_list = [ind.fitness.values for ind in memory_pop]
                fitness_mem = numpy.array(fit_list)
                fit_list2 = [ind.fitness.values for ind in non_dominated_pop]
                fitness_nd= numpy.array(fit_list2)
                distances=cdist(numpy.array(fitness_mem),numpy.array(fitness_nd))
                for mem_i in range(len(memory_pop)):
                     ...
                     index=numpy.argmin(distances[mem_i])
                     # print("Cdist",index2)
                    ...
                    ########################################FLAW remove added dominated
                     ...
                     prob_mem=prob_memory[mem_i]
                     prob_nd=prob_non_dominated[index]

                     # prob_mem2=toolbox.trailing_prob(func=mem_func,df_dict=df_dict,iter=iter,month=month)
                     # prob_nd2=toolbox.trailing_prob(func=func_nd,df_dict=df_dict,iter=iter,month=month)

                     if prob_nd>prob_mem:
                        # logging.warning("Memory updated")
                        memory_pop[mem_i]=non_dominated_pop[index]
                        # sttt=old_non_dominated_pop.pop(index)
                        # new`_memory.append(sttt)
                        sddd=non_dominated_pop.pop(index)
                        p=prob_non_dominated.pop(index)

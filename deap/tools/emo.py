...
def selSPEA2(individuals,k,toolbox):
    ...
    # pareto_indices= [i for i in range(N) if fits[i] < 1]
    # pareto_guys=[individuals[i] for i in chosen_indices]
    ...
    with open('month.pkl', 'rb') as f:
        month = pickle.load(f)
    # with open('env_paretos/' + str(iter_pk) +'_'+str(month)+'_'+'pareto.pkl', 'wb') as f:
    #     pickle.dump(pareto_guys, f)
    ...
    # fig.savefig('plots//'+str(number)+'_'+str(iter_pk)+'_'+str(month)+'.png')
    if len(chosen_indices) < k:  # The archive is too small
        # logging.warning('three for loop 1 start')
        ...
        #     fits[i] += density
        #
        # logging.warning('Start  distance multi if')
        fit_list = [ind.fitness.values for ind in individuals]
        fitness = np.array(fit_list)
        distances_cd = cdist(np.array(fitness), np.array(fitness))
        # fits=toolbox.map(toolbox.loop,((i ,distances_cd ,fits,N,K) for i in range(N)))
        ##THIS is a modified version of distances calculation
        for i in range(N):
            j = i + 1
            temp = [0.0] * j
            ...
            density = 1.0 / (kth_dist + 2.0)
            fits[i] += density
        # logging.warning('End distance multi if')
        ...
        # logging.warning('Else distance start')
        ...
        chosen_individuals=[individuals[i] for i in chosen_indices]
        fit_list = [ind.fitness.values for ind in chosen_individuals]
        fitness = np.array(fit_list)
        distances = cdist(np.array(fitness), np.array(fitness))
        sorted_indices = np.argsort(distances, kind='mergesort', axis=1)
        # logging.warning('Else distance end')
        # logging.warning("Start while cython")
        to_remove=min.minimum(distances, sorted_indices, N, k)
        # logging.warning("End while cython")
        # logging.warning("Start while ")
        # while size > k:
        ...
        #     size -= 1
        # logging.warning("End while ")
        # print(to_remove)
        # logging.warning("deleting indices start")
        for index in reversed(sorted(to_remove)):
            del chosen_indices[index]
        # logging.warning('deleting indices end')
    logging.warning("%s  month  %s  iter",str(month),str(iter_pk))
    pareto_guys=[individuals[i] for i in chosen_indices]
    with open('env_paretos/' + str(iter_pk) +'_'+str(month)+'_'+'pareto.pkl', 'wb') as f:
        pickle.dump(pareto_guys, f)
